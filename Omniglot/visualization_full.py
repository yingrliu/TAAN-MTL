...
model = TAAN(c=0.0, basis=32, lr=1e-5)
...
if __name__ == '__main__':
    ...
    for REG in Categories:
        for k in range(10, 45, 1):
            savedModelPath = os.path.join(REG, 'TAAN-Lambda-{:.2f}'.format(0.01 * k))
            if not os.path.exists(savedModelPath):
                continue
            print(savedModelPath)
            model.loadModel(os.path.join(savedModelPath, 'model_params.pt'))
            Euclidean = [LossDis(model.alphas[i], model.b[i])[1].data.cpu().numpy()
            ...
            axarr[1, 2].axis('off')
            plt.close()
            savedPath = os.path.join('Images2N', REG + '-TAAN-Lambda-{:.2f}.pdf'.format(0.01 * k))
